...
This script handles translation entries that are owned by Thorium directly
(e.g., feature-patch messages that don't exist in upstream Chromium and thus
have no upstream translations to copy). It reads a TSV inventory of additions
and inserts them into the right XTB files with branding replacements applied.

Usage:
  # Dry run
  python3 patch_scripts/grd_rebase/merge_thorium_xtb.py <chromium_root> --dry-run

  # Apply
  python3 patch_scripts/grd_rebase/merge_thorium_xtb.py <chromium_root>
"""
...
def replace_outside_markup(text: str, *, preserve_chrome: bool = False) -> str:
    ...
    replacer = (
        branding_replace_preserving_chrome
        if preserve_chrome
        else branding_replace
    )
    parts: list[str] = []
    cursor = 0
...
def replace_outside_markup_preserving_chromium_link(
    text: str, *, preserve_chrome: bool = False
) -> str:
...
def load_inventory(path: Path) -> list[dict[str, str]]:
    """Load a TSV inventory file of XTB additions."""
...
def group_inventory(
    ...
    """Group inventory rows by target XTB path and validate."""
    ...
    for row in rows:
        ...
        block = normalize_xtb_translation_block(
            row.get("translation_block", ""), translation_id
        )
        if (
        ...
        grouped[target_path].append(
            XtbAddition(translation_id=translation_id, block=block)
        )
    if not grouped:
...
def merge_additions_into_text(
    ...
    return target_text, len(pending), skipped_count, replaced_count
...
def build_merge_results(
    ...
    for target_path, additions in additions_by_target:
        ...
        merged_text, inserted_count, skipped_count, replaced_count = (
            merge_additions_into_text(target_text, additions, target_path)
        )
        results.append(
            XtbMergeResult(
...
def write_merge_results(chromium_root: Path, results: list[XtbMergeResult]) -> None:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--inventory",
        type=readable_file,
        help="Normalized TSV inventory; defaults to the reviewed additions inventory.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    if not inventory.is_file():
        print(
            f"info: no inventory file at {inventory}; nothing to merge",
            file=sys.stderr,
        )
        return 0
